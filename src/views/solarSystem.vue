...
export default {
  ...
  methods: {
    ...
    createSphere(data) {
      ...
      } else if (data.name === '应用风险') {
        this.createBall(data.data)
      } else if (data.name === '应用风险') {
        this.createBall(data.data)
      } else if (data.name === '人员风险') {
        this.createBall(data.data)
      } else {
        //其他球体
    ...
    createTrack(data) {
      ...
      let trackGeometry = new THREE.RingBufferGeometry( //圆环几何体
        data.position[0], //圆环的半径，从圆环的中心到管道（横截面）的中心。默认值是1。
        data.position[0] + 2,//管道的半径，默认值为0.4
        1000//圆环的分段数，默认值为8
      );
      //圆环材质
